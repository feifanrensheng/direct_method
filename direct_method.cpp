...
void DirectPoseEstimationSingleLayer(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (size_t i = 0; i < px_ref.size(); i++) {
            ...
            Eigen::Vector3d p;
            p << px_ref[i](0) * depth_ref[i], px_ref[i](1) * depth_ref[i], depth_ref[i];
            Eigen::Matrix3d K;
            K << fx,0,cx,0,fy,cy,0,0,1;
            Eigen::Vector3d Pc = K.inverse() * p;
            Eigen::Matrix<double,4,1> p_c_1(Pc(0),Pc(1),Pc(2),1);
            Eigen::Matrix<double,4,1> p_c_2 = T21.matrix() * p_c_1;
            Eigen::Vector3d p_uv = K * Eigen::Vector3d(p_c_2(0),p_c_2(1),p_c_2(2));
            double x,y,z;
            x = p_c_2(0);
            y = p_c_2(1);
            z = p_c_2(2);
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    double error =0;
                    error = img1.at<uchar>(px_ref[i](1) + y,px_ref[i](0) + x) - img2.at<uchar>(v + y,u+ x);
                    Matrix26d J_pixel_xi;   // pixel to \xi in Lie algebra
                    Eigen::Vector2d J_img_pixel;    // image gradients
                    J_img_pixel << (img2.at<uchar>(y + 1,x) - img2.at<uchar>(y - 1,x))/2,(img2.at<uchar>(y,x + 1) - img2.at<uchar>(y,x - 1))/2;
